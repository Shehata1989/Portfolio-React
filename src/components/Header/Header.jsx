...
const Header = () => {
  ...
  const [activeLink, setActiveLink] = useState(() => localStorage.getItem("activeLink") || 0)
  ...
  useEffect(() => {
    if (localStorage.getItem("activeLink")) {
      setActiveLink(parseInt(localStorage.getItem("activeLink")));
    }
  }, []);
        ...
        <ul className="hidden bg-[var(--menu-bg)] absolute w-[90%] left-0 right-0 top-20 animate-scale px-10 py-5 md:flex md:justify-center md:items-center md:gap-5 md:static md:px-7 md:py-3 rounded-2xl m-auto md:m-0 md:w-auto">
          {navBar.map((link, idx) => (
            <li className={`mb-3 last:mb-0 md:m-auto`}  key={idx}>
              <a onClick={() => handelLi(idx)}
                className={`${activeLink === idx ? "text-[var(--hover-icon)] font-bold" : ""} hover:text-[var(--hover-icon)] text-[var(--text-color)] hover:font-bold hover:tracking-wide duration-200`}
